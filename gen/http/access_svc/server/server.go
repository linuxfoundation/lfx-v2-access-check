...
// Server lists the access-svc service endpoint HTTP handlers.
type Server struct {
	Mounts              []*MountPoint
...
// New instantiates HTTP handlers for all the access-svc service endpoints
...
func New(
	...
	return &Server{
		Mounts: []*MountPoint{
			...
			{"Serve gen/http/openapi.yaml", "GET", "/_access-check/openapi.yaml"},
			{"Serve gen/http/openapi3.json", "GET", "/_access-check/openapi3.json"},
...
func (s *Server) Service() string { return "access-svc" }
...
func (s *Server) MethodNames() []string { return accesssvc.MethodNames[:] }
...
func Mount(mux goahttp.Muxer, h *Server) {
	...
	MountGenHTTPOpenapiYaml(mux, http.StripPrefix("/_access-check", h.GenHTTPOpenapiYaml))
	MountGenHTTPOpenapi3JSON(mux, http.StripPrefix("/_access-check", h.GenHTTPOpenapi3JSON))
	MountGenHTTPOpenapi3Yaml(mux, http.StripPrefix("/_access-check", h.GenHTTPOpenapi3Yaml))
}
...
// MountCheckAccessHandler configures the mux to serve the "access-svc" service
// "check-access" endpoint.
func MountCheckAccessHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("POST", "/access-check", f)
}

// NewCheckAccessHandler creates a HTTP handler which loads the HTTP request
// and calls the "access-svc" service "check-access" endpoint.
func NewCheckAccessHandler(
	...
	var (
		decodeRequest  = DecodeCheckAccessRequest(mux, decoder)
		encodeResponse = EncodeCheckAccessResponse(encoder)
		encodeError    = EncodeCheckAccessError(encoder, formatter)
	)
...
// MountReadyzHandler configures the mux to serve the "access-svc" service
// "readyz" endpoint.
...
// calls the "access-svc" service "readyz" endpoint.
...
// MountLivezHandler configures the mux to serve the "access-svc" service
// "livez" endpoint.
...
// calls the "access-svc" service "livez" endpoint.
...
func MountGenHTTPOpenapiJSON(mux goahttp.Muxer, h http.Handler) {
	mux.Handle("GET", "/_access-check/openapi.json", h.ServeHTTP)
}
...
func MountGenHTTPOpenapiYaml(mux goahttp.Muxer, h http.Handler) {
	mux.Handle("GET", "/_access-check/openapi.yaml", h.ServeHTTP)
}
...
func MountGenHTTPOpenapi3JSON(mux goahttp.Muxer, h http.Handler) {
	mux.Handle("GET", "/_access-check/openapi3.json", h.ServeHTTP)
}
...
func MountGenHTTPOpenapi3Yaml(mux goahttp.Muxer, h http.Handler) {
	mux.Handle("GET", "/_access-check/openapi3.yaml", h.ServeHTTP)
}
